...
class BaseTest(unittest.TestCase):
    ...
    def test_to_json_string(self):
        ...
        self.assertFalse(isinstance(j, list))
        pobj = json.loads(j)
        self.assertEqual(pobj, [d1, d2])
        self.assertTrue(isinstance(pobj, list))

    def test_from_json_string(self):
        j = '[{"id": 1, "width": 5, "height": 12, "x": 1, "y": 3}, \
            {"id": 3, "width": 6, "height": 3, "x": 8, "y": 2}]'
        dic = Base.from_json_string(j)
        self.assertTrue(isinstance(dic, list))

    def test_none_to_json_string(self):
        """Passing none to empty list"""
    ...
    def test_empty_to_json_string(self):
        ...
        self.assertTrue(isinstance(json_s, str))
        self.assertEqual(json_s, "[]")
#    def tearDown(self):
 #       self.r1.dispose()
  #      self.r2.dispose()
